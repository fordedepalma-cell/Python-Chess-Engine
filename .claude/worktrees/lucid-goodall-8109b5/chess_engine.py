...
class Engine:
    def __init__(self):
        ...
        self.horizontal_squares_for_neg_indices = {
            -v: k for k, v in self.horizontal_indices.items()
        }
        self.vertical_squares_for_neg_indices = {
            -v: k for k, v in self.vertical_indices.items()
        }
    ...
    def get_playable_squares_for_pawn(
        self,
        pawn,
        horizontal_index,
        vertical_index,
        ...
        legal_moves.append(
            self.check_and_add_move(self.square_empty, current_indices, _a, pawn)
        )

        if (
            pawn.rank == starting_rank
            and self.board[horizontal_index + up][vertical_index] is None
        ):
            _b = (horizontal_index + up_2, vertical_index)
            legal_moves.append(
                self.check_and_add_move(self.square_empty, current_indices, _b, pawn)
            )

        _c = (horizontal_index + up, vertical_index - 1)
        legal_moves.append(
            self.check_and_add_move(self.square_capturable, current_indices, _c, pawn)
        )

        _d = (horizontal_index + up, vertical_index + 1)
        legal_moves.append(
            self.check_and_add_move(self.square_capturable, current_indices, _d, pawn)
        )

        return legal_moves

    def get_legal_moves_for_pawn(
        self, pawn, horizontal_index, vertical_index, current_indices, legal_moves
    ):
        if pawn.color == "white":
            return self.remove_all_none_type_vals(
                self.get_playable_squares_for_pawn(
                    pawn,
                    horizontal_index,
                    vertical_index,
                    current_indices,
    ...
    def get_legal_moves_for_knight(
        self, knight, horizontal_index, vertical_index, current_indices, legal_moves
    ):
        possible_moves = [
        ...
        for move in possible_moves:
            legal_moves.append(
                self.check_and_add_move(
                    self.square_playable_or_empty, current_indices, move, knight
                )
            )

        return self.remove_all_none_type_vals(legal_moves)

    def legal_moves_for_bishop(
        self, bishop, horizontal_index, vertical_index, current_indices, legal_moves
    ):
        ...
        for direction in directions_for_bishop:
            ...
            while moves_available:
                ...
                if self.square_playable_or_empty(
                    *move, bishop
                ):  # Bug 1 fix: check candidate move square, not origin
                    legal_moves.append(
                        self.check_and_add_move(
                            self.square_playable_or_empty, current_indices, move, bishop
                        )
                    )
                    if (
                        self.board[move[0]][move[1]] is not None
                    ):  # Bug 2 fix: stop sliding after a capture
    ...
    def legal_moves_for_rook(
        self, rook, horizontal_index, vertical_index, current_indices, legal_moves
    ):
        for hor_step in (-1, 1):
            hor_i = hor_step
            moves_available = True
            while moves_available:
                ...
                if self.square_playable_or_empty(*move, rook):
                    legal_moves.append(
                        self.check_and_add_move(
                            self.square_playable_or_empty, current_indices, move, rook
                        )
                    )
                    if (
                        self.board[move[0]][move[1]] is not None
                    ):  # Bug 4 fix: stop after capture, not after empty square
                        moves_available = False
                else:
                    moves_available = False
                hor_i += hor_step
    ...
    def legal_moves_for_king(
        self, king, horizontal_index, vertical_index, current_indices, legal_moves
    ):
        possible_moves = [
            (horizontal_index, vertical_index - 1),
            (horizontal_index, vertical_index + 1),
            (horizontal_index + 1, vertical_index - 1),
        ...
        for move in possible_moves:
            legal_moves.append(
                self.check_and_add_move(
                    self.square_playable_or_empty, current_indices, move, king
                )
            )
    ...
    def legal_moves_for_a_piece(self, piece):
        ...
        if piece.piece == "pawn":
            ...
            legal_moves = self.legal_moves_for_bishop(
                piece, *compressed_args
            )  # Bug 5 fix: was calling knight method
        elif piece.piece == "rook":
            legal_moves = self.legal_moves_for_rook(piece, *compressed_args)
        elif piece.piece == "queen":
            legal_moves = self.legal_moves_for_rook(
                piece, *compressed_args
            ) + self.legal_moves_for_bishop(piece, *compressed_args)
        elif piece.piece == "king":
            legal_moves = self.legal_moves_for_king(
                piece, *compressed_args
            )  # Bug 7 fix: king moves only, not king + bishop
